# -*- coding: utf-8 -*-
"""
Created on Thu Sep 29 16:16:20 2022
...
df_iteraciones=pd.read_csv("Datos_iteracion_MPC.csv",header=0,index_col=0 )
df_perfil = pd.read_csv('df_perfiles.csv', skiprows=0,header=0,index_col=0)

i=0

if  df_perfil.Disponibilidad_red[i] == 1:
    #instance.PpvIC[0].fix(100)
    instance.PpvIC[0].fix(df_iteraciones.iloc[1,12]) # viene del monitoreo de archivo .dat
    instance.PpvICCurt[0].fix(df_iteraciones.iloc[1,13])

    instance.PpvOnG[0].fix(df_iteraciones.iloc[1,18]) # viene del monitoreo de archivo .dat
    instance.PpvOnGCurt[0].fix(df_iteraciones.iloc[1,19])

    #agregar potencia de carga de las baterias
    instance.Pdch[0].fix(df_iteraciones.iloc[1,21])
    instance.PD[0].fix(df_iteraciones.iloc[1,1])) #validar

elif df_perfil.Disponibilidad_generador[i] == 1:
...
df_resultados_mpc1=pd.read_excel(r"C:\Users\ADMIN\Documents\F. MPC\Datos_MPC_3h.xlsx",header=0,index_col=0 )
df_resultados_mpc1=pd.concat([df_resultados_mpc1, resultados_mpc])
df_resultados_mpc1.to_excel(r"C:\Users\ADMIN\Documents\F. MPC\Datos_MPC_3h.xlsx")
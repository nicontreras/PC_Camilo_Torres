...
if  df_perfil.Disponibilidad_red[i] == 1:
    #instance.PpvIC[0].fix(0)
    #instance.PpvIC[0].fix(350)
    instance.PpvICCurt[0].fix(df_iteraciones.iloc[1,13])
    #instance.SOC[0].fix(df_iteraciones.iloc[1,22])
    #instance.PpvOnG[0].fix(df_iteraciones.iloc[1,18])
    instance.PpvOnGCurt[0].fix(df_iteraciones.iloc[1,19])


    instance.Pdch[0].fix(df_iteraciones.iloc[1,21])
    instance.PD[0].fix(0.0)

elif df_perfil.Disponibilidad_generador[i] == 1:

    instance.PpvIC[0].fix(df_iteraciones.iloc[1,12])
    #instance.PpvICCurt[0].fix(df_iteraciones.iloc[1,13])
    instance.PpvOnG[0].fix(df_iteraciones.iloc[1,18])
    #instance.PpvOnGCurt[0].fix(df_iteraciones.iloc[1,19])
    instance.Pdch[0].fix(df_iteraciones.iloc[1,21])
...
df_resultados_mpc1=pd.read_excel("Datos_MPC_3h.xlsx",header=0,index_col=0 )
df_resultados_mpc1=pd.concat([df_resultados_mpc1, resultados_mpc])
df_resultados_mpc1.to_excel("Datos_MPC_3h.xlsx")